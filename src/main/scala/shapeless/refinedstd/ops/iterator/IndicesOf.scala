...
trait IndicesOf[Count <: Nat, A, Sig] extends DepFn3[Iterator[A], A, Int] with BuiltOutOf[Int] {
  def count: Int

  def apply(it: Iterator[A], elem: A, fromIndex: Int): Out = {
    val b = builder()
    
    def helper(index: Int, remaining: Int): Unit =
      if (remaining > 0 && it.hasNext) {
        if (index >= fromIndex && it.next() == elem) {
          b += index
    ...
    helper(fromIndex, count)
    b.result()
  }
}

object IndicesOf {
  type Aux[Count <: Nat, A, Sig, Out0] = IndicesOf[Count, A, Sig] { type Out = Out0 }

  def apply[Count <: Nat, A, Sig] (implicit 
    indicesOf: IndicesOf[Count, A, Sig]
  ): Aux[Count, A, Sig, indicesOf.Out] = indicesOf

  implicit def indicesOf[Count <: Nat, A, Sig] (implicit
    builder0: typelevel.Builder[Sig, Count, Int],
    count0: ToInt[Count]
  ): Aux[Count, A, Sig, builder0.Out] =
    new IndicesOf[Count, A, Sig] {
